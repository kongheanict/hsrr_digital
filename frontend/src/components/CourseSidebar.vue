    ...
    <div class="parts-list p-4">
      <div v-for="lesson in store.currentCourse?.lessons" :key="lesson.id" class="lesson-group mb-4">
        <h4 @click="toggleLesson(lesson.id)"
            class="lesson-title flex gap-2 items-center py-3 bg-blue-50 pl-2 rounded-sm cursor-pointer hover:bg-gray-100 select-none text-sm font-medium text-gray-700">
            <span class="transition-transform duration-200">{{ expandedLessons[lesson.id] ? '▾' : '▸' }}</span>
          {{ lesson.title }}
        </h4>
        <ul v-if="expandedLessons[lesson.id]" class="mt-2 space-y-1">
          <li v-for="part in lesson.parts" :key="part.id"
           @click.prevent="selectPart(part.id);">
            <a
              ...
              :class="{ 'text-blue-600 font-semibold underline': store.selectedPartId === part.id }"
            >
              {{ part.title }}
            </a>
          </li>
